...
swap_file1    = input("Joint 1: ")
swap_file2    = input("Joint 2: ")
# weights_file = input("Enter Weight File: ")
# model_file = input("Enter YOBJ File: ")

swap_file1    = str.encode(swap_file1)
swap_file2    = str.encode(swap_file2)
...
with open(os.path.join(os.path.dirname(__file__), model_file), 'rb') as model:          #finds bone id
    modelread = model.read()
    model.seek(40)
    bonestart = bytes_to_int((model.read(4))) + 8

    model.seek(bonestart)
    bonelocation = modelread.index(swap_file1)
    file1_boneid = str(int((bonelocation - bonestart) / 80))

    model.seek(bonestart)
    bonelocation = modelread.index(swap_file2)
    file2_boneid = str(int((bonelocation - bonestart) / 80))
